# @shapeshift-labs/frontier-annotations
...
```sh
npm install @shapeshift-labs/frontier-annotations
```

## Browser Overlay

```ts
import { installFrontierAnnotationOverlay } from '@shapeshift-labs/frontier-annotations/browser';

await installFrontierAnnotationOverlay(page, {
...
The overlay installs `window.__FRONTIER_ANNOTATIONS__` with:

- `enableTargeting()` / `disableTargeting()` for DevTools-like hover and click selection.
...
Apps can improve source mapping by adding attributes such as `data-frontier-file`, `data-frontier-line`, `data-frontier-symbol`, `data-component`, or `data-testid` to rendered nodes.
...
const context = createFrontierAnnotationContext(annotation, sourceGraph.sources, {
  maxFiles: 8,
  maxPromptBytes: 48_000
});

const task = createFrontierAnnotationCodexTask(annotation, context, {
  targetRefs: context.sourceRefs.slice(0, 1),
  allowedWrites: context.sourceRefs.slice(0, 1),
  verification: ['npm test'],
  acceptance: ['The annotated UI issue is fixed and covered by evidence.']
});
```

Source records are structural and compatible with Frontier AST-walk source records. Ranking uses direct source hints, component/symbol names, route paths, selectors, test ids, text tokens, imports, tags, and source text.
...
} from '@shapeshift-labs/frontier-annotations/submission';
...
await submitFrontierAnnotationTask(hostSubmitter, submission);
```

The host submitter owns credentials, queues, workspaces, and Codex execution. The package also provides `encodeFrontierAnnotationsJsonl` and `decodeFrontierAnnotationsJsonl` for compact evidence handoff.

## Package Boundary

`frontier-annotations` is an incubation browser/node package. It keeps agent execution, Codex credentials, Playwright, DOM renderers, policy, and persistence outside the package through structural adapters and serializable records.
...
The package benchmark measures annotation-to-source ranking, bounded context creation, and Codex task prompt generation against synthetic source graphs.
