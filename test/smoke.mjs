...
assert.strictEqual(toggle.getAttribute('aria-pressed'), 'true');
assert.strictEqual(window.document.querySelector('[data-frontier-annotation-outline]')?.style.display, 'none');
...
const clickAnnotationClose = overlayRoot.querySelector('[data-frontier-annotation-thread="' + clickAnnotation.id + '"] .frontier-annotation-close');
assert.ok(clickAnnotationClose);
clickAnnotationClose.dispatchEvent(new window.Event('click', { bubbles: true, cancelable: true }));
assert.strictEqual(window[FRONTIER_ANNOTATION_BROWSER_GLOBAL].getAnnotations().some((item) => item.id === clickAnnotation.id), false);
assert.strictEqual(overlayRoot.querySelector('[data-frontier-annotation-thread="' + clickAnnotation.id + '"]'), null);
let composer = overlayRoot.querySelector('[data-frontier-annotation-composer="' + annotation.id + '"]');
assert.ok(composer);
...
textarea.value = 'Draft that should survive thread clicks.';
textarea.dispatchEvent(new window.Event('input', { bubbles: true, cancelable: true }));
textarea.dispatchEvent(new window.MouseEvent('click', { bubbles: true, cancelable: true }));
assert.strictEqual(overlayRoot.querySelector('[data-frontier-annotation-composer="' + annotation.id + '"] textarea'), textarea);
window.dispatchEvent(new window.Event('resize'));
composer = overlayRoot.querySelector('[data-frontier-annotation-composer="' + annotation.id + '"]');
textarea = composer.querySelector('textarea');
assert.strictEqual(textarea.value, 'Draft that should survive thread clicks.');
